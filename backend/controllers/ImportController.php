...
//            die();
    ...
    public function actionDoImport(){
        ...
        $importCsv->table = $_POST['ImportCsv']['table'];
        $importCsv->file = $_POST['ImportCsv']['file'];
        $result = $importCsv->do_import($importCsv->readExcelFile(), $_POST['columns']);
        if($importCsv->table == "Products"){
            $this->actionSetAllShipLedProducts();
        }
        return $this->renderAjax('_import_success', ['result'=>$result, 'model'=> $importCsv]);
    }
